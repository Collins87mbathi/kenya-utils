  ...
  Helpers for Kenya-specific data in JavaScript and TypeScript — phone numbers, KRA PINs, national IDs (including Maisha cards), the 47
   counties, M-PESA, and shilling formatting.
  ...
  [![license](https://img.shields.io/npm/l/kenya-utils.svg)](./LICENSE)
  ...
  Accepts `+254...`, `254...`, `0...`, or a bare 9-digit number. Strips whitespace, hyphens, dots, and parens. Returns `null` on
  garbage rather than throwing.

  A note on network detection: number portability means a "Safaricom-prefixed" number could now be on Airtel. The result is still right
   ~95% of the time, but treat it as a hint, not a guarantee.
  ...
  The Maisha Namba uses the same numeric format as legacy IDs, so `isValidKeId` works for both. The Maisha Card helpers are for the
  card-level concerns the new rollout introduced (mainly: expiry dates).
  ...
  const r = parseMpesaSms(
    "QHJ7K8L9M0 Confirmed. Ksh1,000.00 sent to JOHN DOE 0712345678 on 9/5/26 at 10:30 AM. New M-PESA balance is Ksh5,000.00.
  ...
  The parser handles the common formats — sent, received, paybill, buygoods, withdraw, airtime. Safaricom's SMS wording can shift, so
  it's regex-driven and forgiving. If a format breaks, file an issue with a redacted SMS sample.

  ## What's not here yet

  - **Wards** for each sub-county. There are over 1,400 of them and I want to pull from a verified source rather than type from memory.
   Coming in v1.2.
  ...
  - Sub-county corrections (or wards, when v1.2 lands).
  - Extra M-PESA SMS formats your phone receives that we don't parse.
  - Network prefix updates if Safaricom/Airtel/Telkom adopt new ranges.
