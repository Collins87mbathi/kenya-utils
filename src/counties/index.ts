// 47 counties as of the 2010 constitution. Codes match the official
// county codes (1 = Mombasa through 47 = Nairobi). Region groupings are
// the old pre-2013 provinces — still useful for regional reporting.
//
// subCounties lists the constituencies in each county. In Kenya these
// are often used interchangeably with "sub-county" in everyday language.
...
const byCode = new Map<number, County>(counties.map((c) => [c.code, c]));
const byNameLower = new Map<string, County>(
  counties.map((c) => [c.name.toLowerCase(), c]),
);
const bySlug = new Map<string, County>(counties.map((c) => [c.slug, c]));

export function findCountyByCode(code: number): County | null {
  return byCode.get(code) ?? null;
}

export function findCountyByName(name: string): County | null {
  if (typeof name !== "string") return null;
  return byNameLower.get(name.trim().toLowerCase()) ?? null;
}

export function findCountyBySlug(slug: string): County | null {
  if (typeof slug !== "string") return null;
  return bySlug.get(slug.trim().toLowerCase()) ?? null;
}

export function findCounty(query: string | number): County | null {
...
export function countiesByRegion(region: KenyanRegion): County[] {
  return counties.filter((c) => c.region === region);
}

/** Find which county a sub-county belongs to. Case-insensitive. */
export function findCountyBySubCounty(subCountyName: string): County | null {
  if (typeof subCountyName !== "string") return null;
  const q = subCountyName.trim().toLowerCase();
  return (
    counties.find((c) => c.subCounties.some((s) => s.toLowerCase() === q)) ??
    null
  );
}

/** Flat list of all sub-counties with their parent county code. */
export function allSubCounties(): { name: string; countyCode: number }[] {
  return counties.flatMap((c) =>
    c.subCounties.map((name) => ({ name, countyCode: c.code })),
  );
}
